...
int handle(STACK *s, char *token){
    return add(s,token) || sub(s,token) || mult(s,token) || division(s,token) || mod(s,token) || add1(s,token) || sub1(s,token) || expo(s,token) || e_bit(s,token) || ou_bit(s,token) || xor_bit(s,token) || not_bit(s,token);
    }
